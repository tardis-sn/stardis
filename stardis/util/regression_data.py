...
class RegressionData:
    def __init__(self, request) -> None:
        ...
        if request.config.getoption("--stardis-regression-data") is None:
            pytest.skip("--stardis-regression-data was not specified")
        regression_data_path = Path(
            request.config.getoption("--stardis-regression-data")
        )
    ...
    def sync_hdf_store(self, stardis_module, update_fname=True):
        """
        Synchronizes the HDF store with the regression data.

        Parameters
        ----------
        stardis_module : object
